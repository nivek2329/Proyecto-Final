 ...
 * Gestiona el flujo del juego, la interacción entre entidades y los cambios de estado.
 *
 * @author Angel-Garcia
 * @version 2026-1
 */
public class HardestGame {
    ...
    /**
     * Define los estados posibles en los que puede encontrarse una partida de juego.
     */
    public enum State { 
        /** El juego está activo y en curso. */
        PLAYING, 
        /** El juego se encuentra en pausa. */
        PAUSED, 
        /** El jugador ha completado con éxito el nivel. */
        WON, 
        /** El jugador ha agotado el tiempo o perdido todas las vidas. */
        LOST 
    }
     ...
     * @throws HardestGameException si la skin no está registrada
     */
    private Player createPlayer(final int row, final int col) throws HardestGameException {
        switch (playerSkin) {
            ...
            default:
                ...
                LOGGER.severe(msg);
                throw new HardestGameException(HardestGameException.INVALID_SKIN);
        }
    }
    ...
    public void loadConfiguration(final String filename) throws HardestGameException {
        if (state != State.PAUSED) {
            ...
            LOGGER.severe(msg);
            throw new HardestGameException(HardestGameException.GAME_ALREADY_RUNNING);
        }
     ...
     * @throws HardestGameException si el estado de juego no permite el movimiento
     */
    public void movePlayer(final int deltaRow, final int deltaCol) throws HardestGameException {
        if (state != State.PLAYING) {
            ...
            LOGGER.severe(msg);
            throw new HardestGameException(HardestGameException.INVALID_GAME_STATE);
        }
     ...
     * @param deltaRow desplazamiento en filas
     * @param deltaCol desplazamiento en columnas
     * @throws HardestGameException si no hay dos jugadores activos o si el estado es inválido
     */
     ...
     * @throws HardestGameException si ocurre un error con la moneda
     */
     ...
     *
     * @param activePlayer jugador que recibe el daño
     * @param firstPlayer true si es el jugador principal; false si es el secundario
     * @return true si el daño fue absorbido; false en caso contrario
     */
     ...
     *
     * @param activePlayer jugador que choca
     * @param firstPlayer true si es el jugador principal; false si es el secundario
     * @return true si la colisión fue absorbida; false en caso contrario
     */
     ...
     *
     * @param activePlayer jugador afectado
     * @param firstPlayer true si es el primer jugador
     * @param honorClydeOverlap si true, la inmunidad post-escudo evita otro golpe enemigo
     * @return true si el daño fue mitigado; false si el golpe es letal
     */
    private boolean tryAbsorbDamage(final Player activePlayer, final boolean firstPlayer,
     ...
     * @param firstPlayer true si es el primer jugador
     * @param activePlayer jugador que recibe el daño
     * @param resetCoinsOnFullDeath determina si se reinician las monedas
     */
    ...
    /**
     * Resuelve la zona de reaparición del jugador indicado.
     *
     * @param firstPlayer true para el primer jugador, false para el segundo
     * @return zona de reaparición
     */
     ...
     * @param firstPlayer  true para jugador 1, false para jugador 2
     * @throws HardestGameException si la skin a la cual cambiar es inválida
     */
     ...
     *
     * @param p jugador al que se actualiza la defensa
     */
     ...
     * @param col  columna inicial del jugador
     * @return instancia del jugador creado
     * @throws HardestGameException si la skin a utilizar es inválida
     */
     ...
     * @throws HardestGameException si el reemplazo falla por skin incorrecta
     */
    ...
    /**
     * Retorna la cantidad de vidas extra del jugador 1.
     *
    ...
    /**
     * Retorna una representación en texto del tipo de la zona de reaparición activa.
     *
     * @return "START", "INTERMEDIATE" o "FINAL"
     */
    ...
    /**
     * Resuelve el objeto Zone a partir de su tipo en formato de texto.
     *
     * @param kind cadena de texto indicando el tipo de zona
    ...
    /**
     * Crea un PlayerSnapshot a partir del jugador y su skin.
     *
     * @param p jugador origen
     * @param skin skin activa
     * @return instantánea del jugador
     */
    ...
    /**
     * Crea un EnemySnapshot a partir de un enemigo concreto.
     *
     * @param enemy enemigo origen
    ...
    private EnemySnapshot snapshotForEnemy(final Enemy enemy) {
        ...
        if (enemy instanceof SpinnerEnemy) {
            return ((SpinnerEnemy) enemy).toSnapshot();
        }
        return new EnemySnapshot(enemy.getRow(), enemy.getCol(), 0, 0, 0);
    }

    /**
     * Restaura el estado de un enemigo desde su instantánea.
     *
     * @param enemy enemigo a restaurar
     * @param es instantánea del enemigo
     */
    private void restoreEnemyFromSnapshot(final Enemy enemy, final EnemySnapshot es) {
        if (enemy instanceof BasicEnemy) {
            ...
            ((PatrolEnemy) enemy).restoreSavedState(es.getRow(), es.getCol(),
                    es.getDeltaRow(), es.getTickCount());
        } else if (enemy instanceof SpinnerEnemy) {
            ((SpinnerEnemy) enemy).restoreSavedState(es.getRow(), es.getCol(),
                    es.getDeltaRow(), es.getDeltaCol(), es.getTickCount());
        }
    }

    /**
     * Restaura y crea un jugador a partir de su PlayerSnapshot.
     *
     * @param snap instantánea del jugador
     * @param firstPlayer true para el jugador principal
     * @return instancia de Player restaurada
     * @throws HardestGameException si no se puede crear por skin inválida
     */
