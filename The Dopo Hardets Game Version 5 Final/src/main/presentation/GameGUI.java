 ...
 * Gestiona los eventos de teclado, pantallas de carga, estado y lógica visual.
 *
 * @author Angel-Garcia
 * @version 2026-1
 */
public final class GameGUI extends JFrame {
    ...
    private static String lastConfig = "configs/level1.txt";
    private static GameGUI activeInstance;
     ...
     *
     * @param setup configuración inicial elegida por el usuario
     * @param snapshot partida guardada a restaurar; puede ser null
     */
     ...
     *
     * @param operation nombre o descripción del proceso que falló
     * @param ex excepción atrapada
     */
    private void handleGameError(final String operation, final HardestGameException ex) {
        if (LOGGER.isLoggable(Level.SEVERE)) {
            LOGGER.log(Level.SEVERE, "Error en " + operation + ": " + ex.getMessage(), ex);
        }
        GameLogger.logGameException(operation, ex);
    ...
    /**
     * Crea y añade un botón al panel indicado con el comportamiento especificado.
     *
     ...
     * @return true si la moneda en esa celda está recolectada; false en caso contrario
     */
     ...
     *
     * @return true si hay monedas pendientes; false en caso contrario
     */
     ...
     *
     * @return true si hay al menos un objetivo con moneda válida; false en caso contrario
     */
     ...
     * @param row fila a evaluar
     * @param col columna a evaluar
     * @return true si hay un enemigo adyacente; false en caso contrario
     */
     ...
     * Configura el manejador de excepciones no controladas para logging y reportes automáticos.
     *
     * @param args argumentos de línea de comandos
     */
    public static void main(final String[] args) {
        Thread.setDefaultUncaughtExceptionHandler((thread, throwable) -> {
            GameLogger.logError("Crash/Error no controlado en hilo: " + thread.getName(), throwable);

            SwingUtilities.invokeLater(() -> {
                try {
                    ErrorReportDialog.show(activeInstance, lastConfig, throwable);
                } catch (Exception ex) {
                    if (LOGGER.isLoggable(Level.SEVERE)) {
                        LOGGER.log(Level.SEVERE, "Fallo al mostrar ErrorReportDialog en DefaultUncaughtExceptionHandler: " + ex.getMessage(), ex);
                    }
                }
            });
        });
