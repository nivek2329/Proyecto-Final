...
public abstract class Coin implements GridEntity {
     ...
     *
     * @throws CoinAlreadyCollectedException si ya estaba recolectada
     */
    public void collect() throws CoinAlreadyCollectedException {
        if (collected) {
            final String msg = "Moneda ya recolectada en (" + row + "," + col + ")";
            LOGGER.severe(msg);
            throw new CoinAlreadyCollectedException(msg);
        }
        collected = true;
    }

    /**
     * Marca la moneda como recolectada de forma permanente (checkpoint).
    ...
    /**
     * Indica si la moneda quedó fijada por un checkpoint intermedio.
     *
     * @return true si no debe reaparecer al morir
     */
    ...
    /**
     * Restaura la moneda a estado no recolectado, salvo si es permanente.
     */
    public void reset() {
    ...
    /**
     * Restaura el estado de recolección desde una partida guardada.
     *
     * @param wasCollected indica si la moneda ya había sido recolectada
     * @param wasPermanent indica si la recolección era de carácter permanente
     */
    void restoreSavedState(final boolean wasCollected, final boolean wasPermanent) {
